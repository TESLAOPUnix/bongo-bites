                  ...
                  {recentOrders.map((order) => (
                    <div key={order.id} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                      ...
                      <div className="text-right">
                        <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium capitalize ${order.status === 'delivered' ? 'bg-success/10 text-success' : 'bg-accent/10 text-accent-foreground'}`}>{order.status}</span>
                        <p className="text-sm font-medium mt-1">₹{Number(order.total_amount).toLocaleString('en-IN')}</p>
                      </div>
                    </div>
                  ))}
                  <Link to="/account/orders" className="text-sm text-primary hover:underline">View All Orders →</Link>
                </div>
              ) : (
                <div className="text-center py-8">
                  <Package className="h-12 w-12 text-muted-foreground/30 mx-auto mb-3" />
                  <p className="text-muted-foreground">No orders yet</p>
                  <Link to="/shop" className="text-sm text-primary hover:underline mt-2 inline-block">Start Shopping</Link>
                </div>
              )}
            </div>
          </div>
