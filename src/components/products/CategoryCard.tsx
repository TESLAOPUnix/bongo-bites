...
export default function CategoryCard({ category }: CategoryCardProps) {
  return (
    <Link to={`/category/${category.slug}`} className="category-card group block hover:-translate-y-2 transition-all duration-300">
      <div className="aspect-square overflow-hidden">
        {category.image ? (
          ...
          <div className="w-full h-full bg-secondary flex items-center justify-center"><span className="text-4xl">🛒</span></div>
        )}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5">
        <h3 className="font-display font-semibold text-background text-sm md:text-base mb-1 group-hover:text-primary transition-colors duration-300">{category.name}</h3>
        <div className="flex items-center justify-between">
          <p className="text-xs text-background/70">{category.product_count} products</p>
